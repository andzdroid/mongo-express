...
const routes = function (config) {
  ...
  exp.viewCollection = function (req, res) {
    ...
    const results = runAggregate ? req.collection.aggregate(query) : req.collection.find(query, queryOptions);

    results.toArray(function (err, resultArray) {
      const items = runAggregate ? resultArray && resultArray[0] && resultArray[0].data || [] : resultArray;
      const getCount = runAggregate
        ? (_, __, cb) => cb(
          null,
          resultArray
          && resultArray[0]
        ...
        : (...args) => req.collection.count(...args);
      req.collection.stats(function (err, stats) {
        ...
        req.collection.indexes(function (err, indexes) {
          getCount(query, null, function (err, count) {
            // Pagination
